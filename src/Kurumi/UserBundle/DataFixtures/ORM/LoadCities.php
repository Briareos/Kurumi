...
class LoadCities extends AbstractFixture implements OrderedFixtureInterface, ContainerAwareInterface
{
    ...
    {
        /** @var $cityFinder \Kurumi\UserBundle\City\CityFinderInterface */
        ...
        for ($i = 1; $i <= 500; $i++) {
            /** @var $city City */
            $city = $cityFinder->find('Kurumi\UserBundle\Entity\City', $this->cities[rand(0, count($this->cities) - 1)]);
            if ($city === null) {
                continue;
            }
            if (isset($existingCities[$city->getLatitude() . '-' . $city->getLongitude()])) {
                continue;
            }
            $existingCities[$city->getLatitude() . '-' . $city->getLongitude()] = true;
            $manager->persist($city);
            $this->addReference(sprintf('city_%s', $j), $city);
            $j++;
        }
